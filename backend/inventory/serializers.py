...
class SaleSerializer(serializers.ModelSerializer):
    ...
    def get_items_detail(self,obj):
        return [{'product':i.product_id,'product_name':i.product.name,'quantity':i.quantity,'unit_price':i.unit_price,'subtotal':i.subtotal} for i in obj.items.all()]
    ...
    def create(self,validated_data):
        ...
        for item in items:
            ...
            StockMovement.objects.create(product=product,movement_type=StockMovement.OUT,quantity=qty,note=f'Venda #{sale.id}',created_by=self.context['request'].user)
            total += qty*price
        sale.total=total
